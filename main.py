...
def run_project(args):
    ...
    # Some PyTorch versions do not support the `verbose` argument here.
    try:
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        print(f"Epoch {epoch:03d} | Train Loss: {train_loss:.6f} | Val Loss: {val_loss:.6f} | "
              f"R_in: {model.R_in.item():.4f} | h: {model.h_conv.item():.4f} | "
        ...
        history["r_in"].append(float(model.R_in.item()))
    ...
    if temp_scale is not None and len(y_pred_for_plot) > 0:
        ...
        print(f"🌡️ Test MAE: {test_mae_c:.4f} °C | Test RMSE: {test_rmse_c:.4f} °C | Test MSE: {test_mse_c:.4f} °C²")
    ...
    with open(summary_path, "w", encoding="utf-8") as f:
        ...
        if test_mae_c is not None and test_rmse_c is not None:
            f.write(f"Test MAE (degC): {test_mae_c:.6f}\n")
            f.write(f"Test RMSE (degC): {test_rmse_c:.6f}\n")
        if test_mse_c is not None:
            f.write(f"Test MSE (degC^2): {test_mse_c:.6f}\n")
